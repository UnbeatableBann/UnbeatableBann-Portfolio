...
const FALLBACK_LINKEDIN_POSTS: Article[] = [
  {
    ...
    excerpt: "Building complex AI workflows requires moving past simple chains. I wrote about how we use state graphs (LangGraph-style) to manage state, handle human-in-the-loop validation, and allow agents to route dynamically based on tool feedback. The key lies in state design—specifically, making transitions deterministic while agent behavior remains probabilistic.",
    source: "linkedin",
  ...
  },
  {
    ...
    excerpt: "How do you know if your prompts or retrieval parameters actually improved your RAG output? Blind testing is a recipe for silent failures. In this post, I compare automated metrics (faithfulness, answer relevance) against curated golden sets, explaining when to trust LLM-as-a-judge frameworks and when to rely on traditional assertions.",
    source: "linkedin",
    url: "https://linkedin.com/in/shadab-jamadar",
...
export class LinkedinBlogProvider implements BlogProvider {
  ...
  async fetchArticles(): Promise<Article[]> {
    ...
    try {
      ...
      console.error("[LinkedinBlogProvider] Error reading linkedin-posts.json, using fallback:", error);
      return FALLBACK_LINKEDIN_POSTS;
    }
  }
}
